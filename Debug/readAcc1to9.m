...
%--------------------------------------------------------------------------
function [acc_data, t, Fs ] = readAcc1to9(data_path, d_rate_path, is_disp)
% Created on 06/03/2019 Based on 'readAccRevi.m'
%--------------------------------------------------------------------------
% Configuration
read_num = 10;
half_read = 0.5*read_num; % Must be a integer (= 5)
...
%% Analysis 
if is_disp    
    acc_ind = [1:9]; % Skip Acc 10,20,30,40
    ...
    for k = 1:length(acc_ind)
        for ax = 1:3 % 1:X-axis, 2:Y-axis, 3:Z-axis
            ...
            xlabel({'Time (Secs)';sprintf('%s-axis',axis_label{ax})})
        end  
    end
end
