...
AccIIIDriver::AccIIIDriver() {
    ...
    this->readerThread = std::thread(&AccIIIDriver::readerThread_job2, this); 
}
...
bool AccIIIDriver::readerThread_job() {
    ...
    while (!is_jobdone()) {
        ...
        std::cout << "read_raw: " << std::endl;
        if (0 == read_raw()) {
            continue;
        }

        std::cout << "read_raw: addtoReceivedBytes" << std::endl;
...
bool AccIIIDriver::readerThread_job2() {
    ...
    */
    ...
    std::cout << "ft_open: " << std::endl;
    this->ft_open();
    std::cout << "ft_startCommunication: " << std::endl;
    this->ft_startCommunication();

    std::unique_lock<std::mutex> ulock(this->m_mutex, std::defer_lock);

    do {
        std::cout << "is_jobdone: mlock: " << std::endl;
        ulock.lock();
        jobdone = this->workdone.load();
        ulock.unlock();
        
        if (jobdone) {
            break;
        }
        std::cout << "read_rawIN: " << std::endl;
...
bool AccIIIDriver::ft_open(UCHAR Mask, UCHAR Mode, UCHAR LatencyTimer) {
    DWORD BytesWritten;
...
bool AccIIIDriver::ft_startCommunication(char TxBuffer) {
