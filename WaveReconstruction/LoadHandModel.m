...
%% Plot the hand
if 1 %---------------------------------------------------------------Switch
figure('Position',get(0,'ScreenSize').*[0,0,1,0.92])
% for sim_i = 1:acc_num
for sim_i = 1
    v_color = repmat(Skin_Color,[m_obj.v_num,1]);
%     v_color(:,1) = dist_map(:,sim_i)/sim_radius;
    scatter3(v_posi_X, v_posi_Y, v_posi_Z,[],v_color,'.')
    hold on
...
Z = (-hand_plane(1).*X -hand_plane(2).*Y +1)./hand_plane(3);
surf(X,Y,Z)
end %----------------------------------------------------------------Switch