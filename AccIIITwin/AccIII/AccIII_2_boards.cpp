...
{	
	int DataNum_A = (40000 * 24); // Data Number of accelerometer array A
	...
	fileBuffer_A = new unsigned char[DataNum_A];
	fileBuffer_B = new unsigned char[DataNum_B];
	...
	if ( (ftStatus_A == FT_OK) && (ftStatus_B == FT_OK) )
	{
		...
		SaveDataResult(dwSum_A, fileBuffer_A, "data_A.bin");
		SaveDataResult(dwSum_B, fileBuffer_B, "data_B.bin");
